...
IPAddres=$(ip a s $interfaceName |awk '/inet /{gsub(/\/.*/,"");print $2}')
LANhostName=$(getent hosts $IPAddres | awk '{print $2}')
myExternalIP=$(curl -s icanhazip.com)
myExternalName=$(getent hosts $myExternalIP | awk '{print $2}')
router=$(ip route | grep '^0.0.0.0' | awk '{print $2}')
routerName=$(getent hosts $router | awk '{print $2}')

cat <<EOF
Hostname        : $(hostname)
LAN Address     : $IPAddres
